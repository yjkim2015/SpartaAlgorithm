...
public class IntStack {
    ...
    public int pop() { //시간복잡도 O(n) , 공간복잡도 O(n)
    	int result;
    	if(ptr <= 0) { //배열의 포인터가 0인지 판단
    		...
    		stk[--ptr] = 0; // 마지막배열의 값을 0으로 초기화 및 포인터의 값을 -1 
    	}
    	return result;
    }
    /*
    ...
    public void clear() {
    	...
    	int i = 0;
    	for(int a:stk) { // for문 돌리면서 데이터 0으로 초기화.
    		stk[i] = 0;
    		i++;
    	}
        ptr = 0;
